...
### 현재 상태
- DB 컬럼 — `LocalDateTime` (`Member.createdAt`, `Session.startTime/endTime`, `SessionFeedbackLog.occurredAt` 등) — *timezone 정보 없음*
- 기존 `InternalFeedbackController` — AI 가 보내는 `occurredAt` 의 형식 미명세
- Front — 화면 표시 KST (한국 사용자 대상)
- 서비스가 한국 단일 timezone 운영 가정

### 결정 옵션

| 영역 | 옵션 A | 옵션 B |
|---|---|---|
| API 송수신 표기 | ISO 8601 + `+09:00` (예: `2026-05-25T10:23:45+09:00`) | ISO 8601 + `Z` (UTC, 예: `...10:23:45Z`) |
| DB 저장 | `LocalDateTime` (timezone 없음, KST 가정) | `Instant` / `OffsetDateTime` (UTC) |
| 클라 표시 | KST | KST |

### 추천: **API = +09:00 / DB = LocalDateTime (KST 가정) / 클라 = KST**
- 단일 timezone 서비스 → DB UTC 변환 부담 불필요 ([[project-korean-only]] 정합)
- API 응답에 명시적 `+09:00` 표기로 *클라가 파싱 시 timezone 안 잃음*
- 기존 `LocalDateTime` 컬럼 그대로 활용 — 마이그레이션 불필요
- 글로벌 진출 시 재검토 트리거 (추후 `OffsetDateTime` 전환)

### 결정 후 산출물
- `docs/07-api-design.md` 공통 섹션에 "시간 표기 정책" 명시
- AI 측 batch 송신 시 `occurredAt` 에 `+09:00` 포함하는지 명세
